...
- **Renames happen**: a `clients-project` entry's `title`/`urlTitle`/content can change entirely if the real client project itself gets rebuilt — e.g. id 19 was "Library Management" (`library-management`, a book-borrowing system) until 2026-09-11, when the same real client (Mr. Zaman) had it fully rewritten into "Cloud Flow Library" (`cloud-flow-library`, a paid seat-booking system, unrelated concept). When this happens, `urlTitle` changing means the matching `case-studies.js` entry's `projectUrlTitle` and any DB testimonial's `projectUrlTitle` referencing the old slug must be updated too, or those links silently stop resolving.
...
- Scoped to `type: "clients-project"` entries only (Facelees, Cloud Flow Library — renamed from "Library Management" 2026-09-11, see the Projects section's renames note above) — `saas`/`hobby-project` entries don't get one.
...
### AI Chatbot
- `app/server.js` — `response(prompt, inputOutputPair)` calls Gemini (`gemini-3.6-flash`) via `@google/genai`, using its native **function calling** (tool use), not a single giant context dump. The system prompt (`buildSystemInstruction()`, passed as `config.systemInstruction`) contains: the hand-written About Me / Experience / Education narrative from `app/myself.js`, the full Skills list from `app/about/skills.js`, and a **lightweight Table of Contents** — one line per project (`urlTitle`, title, type, date, `shortDescription` only) built from `app/projects/projects.js`. It deliberately does NOT include each project's full `longDescription`, feature breakdown, or client testimonials up front.
- A `get_project_details` tool is declared alongside it. When a visitor asks something that needs real depth — a project's full description, its feature-by-feature breakdown, its tech stack, or a client's actual testimonial — the model calls this tool with a `urlTitle` from the table of contents; `getProjectDetails()` looks up that one project's full data (including a live `getAllTestimonials()` MongoDB query via `db/queries.js`, matched by `projectUrlTitle`) and the result is fed back to the model in a follow-up turn before it answers. `response()` loops this tool-call round-trip up to `MAX_TOOL_ROUNDS` (3) using the `createUserContent`/`createModelContent`/`createPartFromFunctionResponse` helpers from `@google/genai`. The model's own tool-call turn (`result.candidates[0].content`) is pushed back into `contents` **untouched** — Gemini 3 puts a `thoughtSignature` on each `functionCall` part and rejects the follow-up with a 400 ("Function call is missing a thought_signature") if that turn is rebuilt from `call.name`/`call.args`. The model must also be one the current `GEMINI_API_KEY` can use: keys created after 2026 get 404 "no longer available to new users" on `gemini-2.5-flash`, which is why `MODEL` is `gemini-3.6-flash`. Gemini occasionally returns a transient 503 ("high demand") — the chat then shows "Could not fetch response"; retrying works, no retry logic exists yet. Broad questions (which SaaS products exist, how many projects, etc.) are answered straight from the table of contents with no tool call.
...
>
> **Google OAuth client was replaced 2026-09-20** (new client, new secret; the project number prefix of the client ID changed). The old redirect-URI list was not carried over — the new client in Google Cloud Console has the redirect URIs registered by hand (`http://localhost:3000`–`3005` plus each production domain, always as `<origin>/api/auth/callback/google`). This portfolio's production auth base URL is the **apex** `https://lawrenceamlangomes.com` (NextAuth reports it for both hosts), so only the apex callback is needed — `www` is not. Verified 2026-09-20 that the live site sends the new client to Google and Google accepts it; the final sign-in click was not exercised. In dev, `NEXTAUTH_URL` in `.env.local` is deliberately **commented out**: with `trustHost: true` (`app/auth.js`) NextAuth infers the base URL from the request, whereas a hardcoded port broke login whenever Next.js fell back to a different port. Production's `NEXTAUTH_URL` (Coolify) is unchanged.
>
...
- **`www` → apex redirect is not active.** Coolify's `redirect` setting for this app is `non-www` (saved via API, 2026-09-20), and the app was both redeployed and restarted, yet `https://www.lawrenceamlangomes.com` still serves the site with HTTP 200 instead of redirecting to `https://lawrenceamlangomes.com`. Cause not found (no access to Coolify's proxy labels/UI from a session). Reliable fallback if it still matters: a `redirects()` rule in `next.config.js` matching host `www.lawrenceamlangomes.com` (needs a commit + push).
