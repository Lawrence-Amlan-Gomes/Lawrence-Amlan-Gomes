...
**Database**: MongoDB via Mongoose — connection in `services/mongo.js`. As of 2026-08-27, self-hosted MongoDB on the same Coolify/Hostinger VPS as production (not Atlas — migrated off Atlas this date), sharing the underlying MongoDB instance with the sibling business site **Solvendix**. A scoped, isolated setup was built first (a separate `solvendix_shared` database with its own database-scoped credential, internal-network-only) — Lawrence then explicitly overrode it, later the same day: Solvendix's app now connects **directly to this app's own `lawrenceAmlanGomes` database, using the root credential, over the public address** (`185.201.8.71:27018`, deliberately reopened for this — it had briefly been closed). This gives Solvendix full read/write access to every collection here, not just testimonials — a known, explicit tradeoff Lawrence chose after the risk was raised, not an oversight. See Environment Variables below for connection details and deployment-platform notes.  
...
- **Cross-site sharing with Solvendix (in progress, 2026-08-27)**: testimonials are becoming a genuinely shared collection between this site and Solvendix (both apps reading/writing the same documents, not an API-proxied setup) — see the Database section above for the connection-sharing decision. Three new fields are agreed but **not yet added** to `models/testimonial-model.js` as of this date: `visibleOn: string[]` (which site(s) show a testimonial), `solvendixCaseStudyRef: string | null` (loose reference into Solvendix's own case-study config), `status: enum["pending","approved","rejected"]` (a real moderation gate — doesn't exist today; public submissions currently go live instantly with no approval step). Don't assume these fields exist without checking the model first. Full context: `co-founder/session-log.md`, 2026-08-27 entry.
...
- **MinIO has no CORS configuration — public testimonial photo/video submission is likely broken as of 2026-08-27.** The shared MinIO bucket (`profile-photos`, also used by Solvendix) has no CORS rules at all (`GetBucketCors` → `NoSuchCORSConfiguration`), and this MinIO deployment rejects `PutBucketCors` outright (`501 NotImplemented`) — it can only be fixed at the MinIO container level in Coolify (`MINIO_API_CORS_ALLOW_ORIGIN` env var + restart), not from either app's code. Verified independently, not just reported secondhand. Since `services/s3.js`'s upload flow is a browser-direct presigned POST to this bucket, a visitor attaching a photo/video to a testimonial submission is likely hitting a silent browser-side CORS failure right now. Lawrence is aware, fix deferred to a later session — his call to make in Coolify, not something either app's code can resolve.
